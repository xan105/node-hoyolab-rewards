...
async function checkin(name, token, option = {}){
  ...
  if(info.retcode !== 0) throw new Failure(info.message, { code: info.retcode });
  
  const { data } = info;
  shouldObjLike(data, {
    total_sign_day: isIntegerPositiveOrZero,
    is_sign: isBoolean
  }, new Failure("Unexpected response from the Hoyolab API", 0));
  ...
  if(options.loot){
    const request = await getJSON(url("home"), { maxRetry: 2 });
    if(request.retcode !== 0) throw new Failure(request.message, { code: request.retcode });
